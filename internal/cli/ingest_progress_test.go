...
func TestIngestProgressReporterCombinesKnownAndDiscoveredTotals(t *testing.T) {
	...
	report(ingest.ProgressEvent{Phase: "ingest", Status: "records", Docs: 3, Tokens: 17})
	...
	text := output.String()
	for _, expected := range []string{"0/2 files", "3 docs  17 tokens", "1/2 files", "ingest complete  2/2 files"} {
