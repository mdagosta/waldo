...
func newIndexCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "audit [path]", "Download and validate every indexed record", "Downloads each unique shard through the configured lookaside, validates every canonical record, detects duplicate record IDs, and reconciles manifest totals.", cobra.MaximumNArgs(1), runIndexAudit,
