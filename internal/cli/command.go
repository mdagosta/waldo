...
func newIndexCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "audit [path]", "Verify indexed shards and their ingest attestations", "Downloads each unique shard through the configured lookaside, verifies its object hash, embedded ingest attestation, Parquet schema and aggregate totals, then reconciles manifest totals. Unattested legacy shards fall back to a complete record scan. --deep explicitly revalidates every record hash and token count.", cobra.MaximumNArgs(1), runIndexAudit,
