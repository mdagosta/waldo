	...
	return Command{
		...
		Children: []Command{
			...
			{Name: "lookaside", Summary: "Inspect and maintain content-addressed objects", Children: []Command{
				...
				{Name: "list", Summary: "List lookaside objects and optional index references", Usage: "waldo lookaside list [index-path] [--json]", Details: "For S3, lists every object in the configured lookaside's bucket and marks whether it is beneath the configured write prefix. A file:// lookaside lists its configured root. Object bodies are not downloaded. With an index path, recursively resolves that selected subtree and annotates objects with the corpus manifests that reference their hashes. Objects reported as not in the selected index are not necessarily safe to remove; another index or BOM may still reference them.", Handler: runLookasideList},
