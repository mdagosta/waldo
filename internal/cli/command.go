	...
	return Command{
		...
		Children: []Command{
			{Name: "index", Summary: "Manage indexed training data", Children: []Command{
				...
				{Name: "ingest", Summary: "Ingest acquired material into an index", Usage: "waldo index ingest <input> <destination> --title <title> --license <id> --source <url> --source-category <category> [--description <text>] [--source-name <name>] [--text-column <column>] [--mode streaming|canonical] [--memory <bytes|MiB|GiB>] [--staging <directory>] [--object-base <s3-or-file-url>] [--local-only | --dry-run] [--json]", Handler: runIndexIngest},
			...
			{Name: "lookaside", Summary: "Configure and maintain content-addressed objects", Children: []Command{
				{Name: "configure", Summary: "Configure cache, read mirrors, and publication", Usage: "waldo lookaside configure [--cache <directory> | --default-cache] [--mirror <base-url>] [--publish <s3-or-file-url> | --publish-local <directory>] [--publish-region <region>] [--publish-endpoint <url>] [--publish-path-style] [--upload-workers <1..32>] [--keep-local | --no-keep-local] [--clear-publish] [--json]", Handler: runLookasideConfigure},
