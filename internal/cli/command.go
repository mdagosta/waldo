	...
	return Command{
		...
		Children: []Command{
			...
			{Name: "model", Summary: "Build and inspect provenance-carrying models", Children: []Command{
				...
				{Name: "build", Summary: "Build a model from a declarative compose", Usage: "waldo model build <compose.yaml> [--json]", Details: "Preflights the complete model compose and corpus selections, creates an immutable model plan, and executes its ordered stages. The current development resolver enables only the deterministic fake backend; real training adapters come next. Existing models are never continued or replaced implicitly.", Handler: runModelBuild},
