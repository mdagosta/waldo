...
func newModelCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "train <name> [index-path...] | <name> <compose-file>", "Train a model from indexed corpora or a compose", "A compose creates an absent model or trains an existing model with the same architecture. With no input, WALDO trains an existing model on the entire resolved index. --nodes greater than 1 runs a multi-node TorchTitan job (requires the TorchTitan backend, and currently byte-tokenizer models only; subword models train from a compose file, which is single-node); this invocation is the primary (rank 0), and every other node runs `model train-worker` pointed at the primary's --rendezvous host:port with the same --rendezvous-id run label. The nodes are coupled by --rendezvous; --rendezvous-id only labels the run.", modelTrainArgs, runModelTrain,
			...
			textFlag("rendezvous-id", "", "shared run label; identifies the run, not enforced across nodes (required when --nodes > 1)"),
		...
		leaf(state, "train-worker", "Join a multi-node training run as a secondary node", "Runs on every non-primary node of a multi-node run: it joins the rendezvous started by `model train --nodes N` on the primary and runs this node's local ranks. It authors no model records; the primary owns the run BOM. --node-rank is this node's rank in 1..N-1. --rendezvous must be the primary's host:port (this is what couples the nodes); --rendezvous-id is a shared run label matching the primary's.", cobra.NoArgs, runModelTrainWorker,
			integerFlag("nodes", 2, "total nodes in the run (>= 2)"),
			integerFlag("node-rank", 0, "this node's rank (1..nodes-1)"),
			textFlag("rendezvous", "", "the primary's host:port the nodes rendezvous on"),
			textFlag("rendezvous-id", "", "shared run label; use the primary's --rendezvous-id")),
