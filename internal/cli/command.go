...
func commandTree() Command {
	return Command{
		...
		Children: []Command{
			{Name: "fetch", Summary: "Acquire upstream data into a reviewable local deposit", Details: "Fetchers stop after writing local bytes and schema-1 acquisition evidence. They never ingest, publish to a lookaside, mutate an index, or start a model operation.", Children: []Command{
				{Name: "huggingface", Summary: "Fetch one immutable Hugging Face dataset file", Usage: "waldo fetch huggingface <owner/dataset> <file> <directory> [--revision <revision>] [--json]", Details: "Arguments:\n  <owner/dataset>  Hugging Face dataset repository.\n  <file>           Exact repository-relative file to acquire.\n  <directory>      Explicit local deposit directory.\n\nOption:\n  --revision       Branch, tag, or commit to resolve; defaults to main. The resulting acquisition record always pins the resolved commit.\n\nThe selected file must expose an upstream LFS SHA-256 and size. WALDO streams it without conversion, verifies it, and writes ACQUISITION.json last. HF_TOKEN is read from the environment for gated datasets and is never persisted.", Handler: runFetchHuggingFace},
			}},
			{Name: "index", Summary: "Manage indexed training data", Details: "Index paths are positional. Existing checkout, subtree, corpus-directory, and manifest paths discover their enclosing checkout automatically. With no path, WALDO discovers from the current directory. Recursive commands begin at the selected path.", Children: []Command{
				...
				{Name: "ingest", Summary: "Ingest acquired material into an index", Usage: "waldo index ingest <input> <destination> --license <id> [--title <title>] [--source <url>] [--source-category <category>] [--description <text>] [--source-name <name>] [--text-column <column>] [--dry-run] [--json]", Details: "Required:\n  <input>             File, recursively scanned directory, or WALDO acquisition deposit.\n  <destination>       New corpus path. An absolute or ./ path discovers its checkout; a logical path uses the checkout containing the current directory.\n  --license           Curated license applying to this contribution.\n\nWithout ACQUISITION.json:\n  --title             Human-readable corpus title.\n  --source            Acquisition/source URL recorded in provenance.\n  --source-category   GPAI-compatible source category.\n\nOptional:\n  --title             Overrides the acquisition proposal.\n  --description       Overrides the acquisition proposal; WALDO generates a default otherwise.\n  --source-name       Overrides the acquisition source label.\n  --source             Overrides the recorded source URL.\n  --source-category   Overrides the recorded acquisition category.\n  --text-column       Raw-Parquet text column when it cannot be inferred.\n  --dry-run           Probe and print the immutable plan without writing.\n\nA verified acquisition supplies title, description, source URL, category, version, acquisition month, artifact order, hashes, and upstream URLs. Set machine locations with `waldo config set`; ingestion has no transport or scratch flags.", Handler: runIndexIngest},
