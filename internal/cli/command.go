	...
	return Command{
		...
		Children: []Command{
			{Name: "index", Summary: "Manage indexed training data", Children: []Command{
				...
				{Name: "ingest", Summary: "Ingest acquired material into an index", Usage: "waldo index ingest <input> <destination> --title <title> --license <id> --source <url> --source-category <category> [--description <text>] [--source-name <name>] [--text-column <column>] [--mode streaming|canonical] [--memory <bytes|MiB|GiB>] [--dry-run | --staging <directory> --object-base <url>] [--json]", Handler: runIndexIngest},
