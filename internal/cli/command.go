...
func newModelCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "advisor <name>", "Design a validated follow-up compose", "Reviews the saved compose and training evidence, asks about the next model's goal and compute budget in a terminal, then uses the configured AI provider to write a new validated compose. It never changes the source model or overwrites an existing file.", cobra.ExactArgs(1), runModelAdvisor,
			textFlag("provider", "", "AI provider override: auto, openai, or anthropic"),
			textFlag("model", "", "provider API model ID override"),
			textFlag("goal", "", "intended behavior or outcome for the next model"),
			textFlag("budget", "", "hardware and wall-clock budget for the next training run"),
			textFlag("priority", "", "quality or behavior to improve first"),
			textFlag("output", "", "new compose path; default <name>-advisor.yaml")),
