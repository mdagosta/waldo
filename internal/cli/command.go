	...
	return Command{
		...
		Children: []Command{
			...
			{Name: "lookaside", Summary: "Inspect and maintain content-addressed objects", Children: []Command{
				...
				{Name: "list", Summary: "List lookaside objects and optional index references", Usage: "waldo lookaside list [index-path] [--json]", Details: "Lists every object beneath the configured writable lookaside prefix without downloading object bodies. With an index path, recursively resolves that selected subtree and annotates objects with the corpus manifests that reference their hashes. Objects reported as not in the selected index are not necessarily safe to remove; another index or BOM may still reference them.", Handler: runLookasideList},
