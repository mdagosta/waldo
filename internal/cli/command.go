	...
	return Command{
		...
		Children: []Command{
			...
			{Name: "model", Summary: "Create, train, compose, and use auditable models", Children: []Command{
				...
				{Name: "train", Summary: "Train an existing model on indexed corpora", Usage: "waldo model train <name> <index-path...> [--json]", Details: "Recursively resolves and deduplicates index selections, materializes hash-verified shards through the configured cache, audits every canonical record, and appends one explicit training run. The current backend remains deterministic and simulated.", Handler: runModelTrain},
				...
				{Name: "chat", Summary: "Chat with a trained local model", Usage: "waldo model chat <name>", Details: "Validates the named model. Interactive generation becomes available when the real MLX backend produces chat-capable weights.", Handler: runModelChat},
