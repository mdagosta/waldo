	...
	return Command{
		...
		Children: []Command{
			{Name: "index", Summary: "Manage indexed training data", Details: "Index paths are positional. Existing checkout, subtree, corpus-directory, and manifest paths discover their enclosing checkout automatically. With no path, WALDO discovers from the current directory. Recursive commands begin at the selected path.", Children: []Command{
				...
				{Name: "ingest", Summary: "Ingest acquired material into an index", Usage: "waldo index ingest <input-or-compose> <destination> [metadata options] [--dry-run] [--json]", Details: "Arguments:\n  <input-or-compose>  A file, recursively scanned directory, or strict waldo-ingest-compose YAML/JSON file.\n  <destination>       New corpus path. An absolute or ./ path discovers its checkout; a logical path uses the checkout containing the current directory.\n\nRequired: direct input only\n  --title             Human-readable corpus title.\n  --license           License applying to this contribution.\n  --source            Acquisition/source URL recorded in provenance.\n  --source-category   GPAI-compatible source category.\n\nDirect input options:\n  --description       Corpus description; WALDO generates a default otherwise.\n  --source-name       Source label; defaults from the destination name.\n  --text-column       Raw-Parquet text column when it cannot be inferred.\n\nUniversal option:\n  --dry-run           Direct input is probed; compose input resolves and hashes commands but never executes them.\n\nA compose owns all corpus metadata and rejects metadata flags. Each step uses `exec`: a bare command resolves through PATH, while a value containing a path separator is explicit and relative to the compose file unless absolute. Commands run directly and sequentially with one WALDO-owned temporary directory as their working directory and WALDO_FETCH_DIR; no shell is implied. WALDO then uses the normal probe, conversion, publication, purge, and contribution path. Set machine locations with `waldo config set`; ingestion has no transport or scratch flags.", Handler: runIndexIngest},
