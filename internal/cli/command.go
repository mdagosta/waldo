...
func newIndexCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "update <input-or-recipe> <manifest>", "Append or completely rebuild an existing corpus", "Normal update audits existing shards and publishes only new records. --rebuild-shards treats the supplied input as the complete authoritative corpus. Fetcher handoff directories use their manifest metadata and input mappings in either mode.", cobra.ExactArgs(2), runIndexUpdate, append(ingestFlags(), booleanFlag("rebuild-shards", "replace the existing shard and source set"))...),
