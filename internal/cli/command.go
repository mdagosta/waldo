...
func newModelCommand(state *cobraState) *cobra.Command {
	...
	command.AddCommand(
		...
		leaf(state, "advise <name> [question]", "Assess a model or training run", "Reads the saved compose, current run state, and training telemetry. Always produces deterministic findings; a configured AI provider adds contextual recommendations. This command never changes or stops a run.", cobra.RangeArgs(1, 2), runModelAdvise,
			textFlag("provider", "", "AI provider override: auto, openai, anthropic, deterministic, or local"),
			textFlag("model", "", "AI provider model override")),
