...
// Resolve locates one index checkout and a target inside it. explicitRoot is
// the value of --index when provided. Otherwise WALDO discovers the checkout
// from the current directory or from an explicitly filesystem-shaped target.
func Resolve(explicitRoot, target string) (Target, error) {
	if explicitRoot != "" {
		root, err := findRoot(explicitRoot)
		if err != nil {
			return Target{}, fmt.Errorf("--index %s: %w", explicitRoot, err)
		}
		if target == "" {
	...
	if err != nil {
		return Target{}, fmt.Errorf("current directory: %w (use --index <checkout>)", err)
	}
	if target == "" {
		return Target{Root: root, Abs: root}, nil
	}
